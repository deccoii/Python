...
def possibilités(mouvement_possible,nom_piece,pos_pieces):
    for key in pos_pieces.keys() :
        if key[1] == "r" :
            moove = []
            for i in range(8) :
                moove.append((pos_pieces[key][0],i*100))
            for i in range(8) :
                moove.append((i*100,pos_pieces[key][1]))
            mouvement_possible[key] = moove
        if key[1] == "b" :
            moove = []
            for i in range(8) :
                moove.append((pos_pieces[key][0]+i*100,pos_pieces[key][1]+i*100))
            for i in range(8) :
                moove.append((pos_pieces[key][0]+i*100,pos_pieces[key][1]-i*100))
            for i in range(8) :
                moove.append((pos_pieces[key][0]-i*100,pos_pieces[key][1]+i*100))
...
while continuer:##Boucle de jeu
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:##Si on appuis sur la croix ferme le jeu et l'arrete
            ...
            if int(joueur) % 2 == 0 :
                if nom_piece[0] == "w" :
                    if (pos_pieces[nom_piece] in mouvement_possible[nom_piece]) :
                        mouvement(nom_piece,nom_piece2,joueur)
